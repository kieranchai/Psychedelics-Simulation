...
public class FirstPersonController : MonoBehaviour
{
    ...
    public bool subtleSwayEnabled = false;
    private float swaySpeed = 1f;
    private float swayMagnitude = 5f;
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            ...
            if (subtleSwayEnabled)
            {
                float swayAmount = Mathf.Sin(Time.time * swaySpeed) * swayMagnitude;
                float swayDirection = Mathf.Sin(Time.time * swaySpeed * 0.75f); // Adjust speed for direction
                Vector3 swayOffset = new Vector3(swayAmount * swayDirection, 0, 0);

                transform.localEulerAngles += swayOffset;
            }
